...
import org.cordovastudio.editors.designer.actions.*;
...
public class RadLinearLayout extends RadViewLayoutWithData implements ILayoutDecorator {
    ...
    public boolean isHorizontal() {
        return !VALUE_VERTICAL.equals(((RadViewComponent) myContainer).getTag().getAttributeValue(ATTR_ORIENTATION, CORDOVASTUDIO_URI));
    }
    ...
    private class GravityAction extends AbstractGravityAction<Gravity> {
        ...
        public GravityAction(CordovaDesignerEditorPanel designer, List<? extends RadViewComponent> components) {
    ...
    public void wrapIn(final RadViewComponent newParent, final List<RadViewComponent> components) throws Exception {
        ...
        String layoutWidth = single ? firstComponent.getTag().getAttributeValue("layout_width", NS_RESOURCES) : "wrap_content";
        String layoutHeight = single ? firstComponent.getTag().getAttributeValue("layout_height", NS_RESOURCES) : "wrap_content";
        String layoutGravity = firstComponent.getTag().getAttributeValue("layout_gravity", NS_RESOURCES);

        if (horizontal) {
            for (RadViewComponent component : components.subList(1, components.size())) {
                String height = component.getTag().getAttributeValue("layout_height", NS_RESOURCES);
                if ("fill_parent".equals(height) || "match_parent".equals(height)) {
                    layoutHeight = "fill_parent";
                    layoutGravity = null;
                }
                if (layoutGravity != null &&
                        layoutGravity.equals(component.getTag().getAttributeValue("layout_gravity", NS_RESOURCES))) {
        ...
        if (newParent.getLayout() instanceof RadLinearLayout) {
            ...
            if (horizontal != layout.isHorizontal()) {
                ApplicationManager.getApplication().runWriteAction(new Runnable() {
                    @Override
                    public void run() {
                        newParent.getTag().setAttribute("orientation", NS_RESOURCES, horizontal ? "horizontal" : "vertical");
                    }
                });
            }
        } else {
