...
public class RadComponentOperations {
  ...
  private static void addComponent(RadViewComponent parentComponent,
                                   MetaManager metaManager) throws Exception {
    for (XmlTag tag : parentComponent.getTag().getSubTags()) {
      ...
      addComponent(component, metaManager);
    }
  }
  ...
  private static void pasteComponent(final RadViewComponent component,
                                     XmlTag parentTag,
                                     @Nullable XmlTag nextTag) throws Exception {
    addComponentTag(parentTag, component, nextTag, new Computable<String>() {
      @Override
      public String compute() {
        Element pasteProperties = component.extractClientProperty(CordovaPasteFactory.KEY);
    ...
    for (int i = 0; i < size; i++) {
      ...
      pasteComponent(child, xmlTag, nextChildTag);
    }
  }
    ...
    ApplicationManager.getApplication().runWriteAction(new Runnable() {
      ...
      public void run() {
        ...
        if (checkTag(parentTag)) {
          String namespacePrefix = parentTag.getPrefixByNamespace(GlobalConstants.CORDOVASTUDIO_URI);
          // In the metadata the namespace prefix is hardcoded as "android"; convert to the current file's namespace
          if (namespacePrefix != null && !GlobalConstants.CORDOVASTUDIO_NS_NAME.equals(namespacePrefix)) {
          ...
          xmlTag.setAttribute(GlobalConstants.XMLNS, GlobalConstants.CORDOVASTUDIO_URI);
          if (xmlFile != null) {
  ...
  private static void convertNamespacePrefix(XmlTag xmlTag, String namespacePrefix) {
    for (XmlAttribute attribute : xmlTag.getAttributes()) {
      if (GlobalConstants.CORDOVASTUDIO_NS_NAME.equals(attribute.getNamespacePrefix())) {
  ...
  public static void deleteAttribute(XmlTag tag, String name) {
    deleteAttribute(tag, name, GlobalConstants.CORDOVASTUDIO_URI);
  }
