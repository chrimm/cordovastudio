...
import org.cordovastudio.editors.designer.designSurface.ScalableComponent;
import org.cordovastudio.editors.designer.designSurface.TransformedComponent;
...
public class RadViewComponent extends RadVisualComponent {
  ...
  @Override
  public CordovaMetaModel getMetaModel() {
    return (CordovaMetaModel)super.getMetaModel();
  }
  ...
  @Nullable
  public String getId() {
    return StringUtil.nullize(getTag().getAttributeValue(ATTR_ID, CORDOVASTUDIO_URI), false);
  }
  ...
  public boolean calculateWrapSize(@NotNull Dimension wrapSize, @Nullable Rectangle bounds) {
    ...
    @Nullable
    public Dimension calculateWrapSize(IEditableArea area) {
        if (myViewInfo != null) {
        ...
        if (parentTag != null) {
            RenderService service = CordovaDesignerUtils.getRenderService(area);
            if (service == null) {
                return null;
            }
            ...
            ViewInfo viewInfo = service.measureChild(getTag(), new RenderService.AttributeFilter() {
                ...
                public String getAttribute(@NotNull XmlTag n, @Nullable String namespace, @NotNull String localName) {
                    if ((ATTR_LAYOUT_WIDTH.equals(localName) || ATTR_LAYOUT_HEIGHT.equals(localName)) && CORDOVASTUDIO_URI.equals(namespace)) {
                        return VALUE_WRAP_CONTENT;
                    } else if (ATTR_LAYOUT_WEIGHT.equals(localName) && CORDOVASTUDIO_URI.equals(namespace)) {
                        return ""; // unset
                    }
   ...
   * the amount of rounding error.
   *
   * @deprecated This method souldn't be needed as in Cordova we are using px everywhere instead of dp!
   */
