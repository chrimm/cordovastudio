...
public class LinearLayoutResizeOperation extends ResizeOperation {
  ...
  protected static boolean isVertical(@NotNull RadViewComponent node) {
    ...
    return VALUE_VERTICAL.equals(tag.getAttributeValue(ATTR_ORIENTATION, CORDOVASTUDIO_URI));
  }
    ...
    if (resizeContext.myUseWeight) {
      ...
      if (!isVertical && horizontalEdge != null) {
        if (newBounds.height != oldBounds.height || resizeContext.wrapHeight || resizeContext.fillHeight) {
          node.setAttribute(ATTR_LAYOUT_HEIGHT, CORDOVASTUDIO_URI, resizeContext.getHeightAttribute());
        }
      }
      if (isVertical && verticalEdge != null) {
        if (newBounds.width != oldBounds.width || resizeContext.wrapWidth || resizeContext.fillWidth) {
          node.setAttribute(ATTR_LAYOUT_WIDTH, CORDOVASTUDIO_URI, resizeContext.getWidthAttribute());
        }
      }
    } else {
      node.setAttribute(ATTR_LAYOUT_WEIGHT, CORDOVASTUDIO_URI, null);
  ...
  private static float getWeight(RadViewComponent linearLayoutChild) {
    String weight = linearLayoutChild.getTag().getAttributeValue(ATTR_LAYOUT_WEIGHT, CORDOVASTUDIO_URI);
  ...
  private static float getWeightSum(RadViewComponent linearLayout) {
    String weightSum = linearLayout.getTag().getAttributeValue(ATTR_WEIGHT_SUM, CORDOVASTUDIO_URI);
    if (weightSum != null) {
      // Distribute
      try {
  ...
  private class LinearResizeContext extends ResizeContext {
    ...
    private LinearResizeContext(IEditableArea area, RadViewComponent layout, @Nullable Object layoutView, RadViewComponent node) {
    ...
    private Map<XmlTag, Dimension> computeUnweightedSizes() {
      ...
      if (parentTag != null) {
        RenderService service = CordovaDesignerUtils.getRenderService(myContext.getArea());
        if (service == null) {
          return null;
        }
        ...
        Map<XmlTag, ViewInfo> map = service.measureChildren(parentTag, new RenderService.AttributeFilter() {
          ...
          public String getAttribute(@NotNull XmlTag n, @Nullable String namespace, @NotNull String localName) {
            ...
            // of the children
            if (ATTR_LAYOUT_WEIGHT.equals(localName) && CORDOVASTUDIO_URI.equals(namespace)) {
              return ""; //$NON-NLS-1$
            }
    ...
    public void apply() {
      ...
      node.setAttribute(ATTR_LAYOUT_WEIGHT, CORDOVASTUDIO_URI, value);

      if (myClearWeights != null) {
        for (RadViewComponent n : myClearWeights) {
          if (getWeight(n) > 0.0f) {
            n.setAttribute(ATTR_LAYOUT_WEIGHT, CORDOVASTUDIO_URI, null);
          }
        }
      }

      if (myNewWeightSum > 0.0) {
        layout.setAttribute(ATTR_WEIGHT_SUM, CORDOVASTUDIO_URI, XmlUtils.formatFloatAttribute(myNewWeightSum));
      }
    }
  }
}
