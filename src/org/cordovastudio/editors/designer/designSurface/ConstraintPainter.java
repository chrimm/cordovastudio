...
public class ConstraintPainter {
  ...
  public static void paintSelectionFeedback(DesignerGraphics graphics,
    ...
    Set<RadViewComponent> verticalDeps = dependencyGraph.dependsOn(childNodes, true /* vertical */);
    ...
    if (deps.size() > 0) {
      for (RadViewComponent node : deps) {
        // Don't highlight the selected nodes themselves
        if (childNodes.contains(node)) {
          continue;
        }
        Rectangle bounds = node.getBounds(graphics.getTarget());
        graphics.useStyle(DEPENDENCY);
        graphics.fillRect(bounds.x, bounds.y, bounds.width, bounds.height);
      }
    }

    for (RadViewComponent childNode : childNodes) {
   ...
   */
  private static boolean paintCornerConstraint(DesignerGraphics graphics,
                                               ConstraintType type,
                                               RadViewComponent sourceNode,
                                               Rectangle sourceBounds,
                                               RadViewComponent targetNode,
   ...
   * </pre>
   */
  private static void paintVerticalConstraint(DesignerGraphics graphics,
                                              ConstraintType type,
                                              RadViewComponent sourceNode,
                                              Rectangle sourceBounds,
                                              RadViewComponent targetNode,
    ...
    int minRight = Math.min(GuidelineHandler.x2(sourceBounds), GuidelineHandler.x2(targetBounds));

    int center = (maxLeft + minRight) / 2;
    if (center > sourceBounds.x && center < GuidelineHandler.x2(sourceBounds)) {
      ...
      if (targetSegmentTypeY == SegmentType.BOTTOM && targetMargins.bottom > 5) {
        ...
        if (sourceY > sharedY + 2) { // Skip when source falls on the margin line
          ...
          graphics.drawLine(targetBounds.x, sharedY, GuidelineHandler.x2(targetBounds), sharedY);
          graphics.useStyle(GUIDELINE);
   ...
   */
  private static void paintHorizontalConstraint(DesignerGraphics graphics,
                                                ConstraintType type,
                                                RadViewComponent sourceNode,
                                                Rectangle sourceBounds,
                                                RadViewComponent targetNode,
    ...
    int minBottom = Math.min(GuidelineHandler.y2(sourceBounds), GuidelineHandler.y2(targetBounds));
    ...
    if (center > sourceBounds.y && center < GuidelineHandler.y2(sourceBounds)) {
      ...
      if (targetSegmentTypeX == SegmentType.RIGHT && targetMargins.right > 5) {
        ...
        if (sourceX > sharedX + 2) { // Skip when source falls on the margin line
          graphics.useStyle(GUIDELINE_DASHED);
          graphics.drawLine(sharedX, targetBounds.y, sharedX, GuidelineHandler.y2(targetBounds));
          graphics.useStyle(GUIDELINE);
  ...
  private static void paintVerticalCenterConstraint(DesignerGraphics graphics, Rectangle sourceBounds, Rectangle targetBounds) {
    graphics.useStyle(GUIDELINE_DASHED);
    graphics.drawLine(targetBounds.x, GuidelineHandler.centerY(targetBounds), GuidelineHandler.x2(targetBounds), GuidelineHandler
      .centerY(targetBounds));
    graphics.useStyle(GUIDELINE);
    graphics.drawLine(sourceBounds.x, GuidelineHandler.centerY(sourceBounds), GuidelineHandler.x2(sourceBounds), GuidelineHandler
  ...
  private static void paintHorizontalCenterConstraint(DesignerGraphics graphics, Rectangle sourceBounds, Rectangle targetBounds) {
    graphics.useStyle(GUIDELINE_DASHED);
    graphics.drawLine(
      GuidelineHandler.centerX(targetBounds), targetBounds.y, GuidelineHandler.centerX(targetBounds), GuidelineHandler.y2(targetBounds));
    graphics.useStyle(GUIDELINE);
    graphics.drawLine(
      GuidelineHandler.centerX(sourceBounds), sourceBounds.y, GuidelineHandler.centerX(sourceBounds), GuidelineHandler.y2(sourceBounds));
  }
}
