...
import static org.cordovastudio.editors.designer.rendering.renderConfiguration.RenderConfigurationListener.MASK_RENDERING;
...
public class RenderPreview implements Disposable {
    ...
    private
    @NotNull
    RenderConfiguration myConfiguration;
    ...
    private
    @NotNull
    RenderConfiguration myAlternateConfiguration;
    ...
    private final
    @NotNull
    RenderPreviewManager myManager;
     ...
     * {@link org.cordovastudio.editors.designer.rendering.renderConfiguration.RenderConfigurationListener}.
     */
    private int myDirty;
    ...
    private void updateForkStatus() {
        /* TODO: reimplement for Cordova Studio
        ...
        if (editedFile != null) {
            if (!myConfiguration.isBestMatchFor(editedFile, config)) {
                ...
                if (resources != null) {
                    ...
                    if (myAlternateInput != null) {
                        myAlternateConfiguration = RenderConfiguration.create(myConfiguration, myAlternateInput);
                    }
                }
            }
        }
     ...
     */
    @NotNull
    public static RenderPreview create(@NotNull RenderPreviewManager manager, @NotNull RenderConfiguration configuration, boolean showFrame) {
    ...
    private boolean tryRenderSync() {
        ...
        final RenderConfiguration configuration = myAlternateInput != null && myAlternateConfiguration != null
                ? myAlternateConfiguration : myConfiguration;
        PsiFile psiFile;
        if (myAlternateInput != null) {
            psiFile = CordovaPsiUtils.getPsiFileSafely(module.getProject(), myAlternateInput);
        } else {
    ...
    void createThumbnail() {
        ...
        PreviewToolWindowSettings.GlobalState settings =
                PreviewToolWindowSettings.getInstance(project).getGlobalState();
    ...
    void paint(Graphics2D gc, int x, int y) {
        ...
        if (thumbnail != null && myError == null) {
            ...
            if (myActive) {
                ...
                if (myConfiguration.getDevice().isRound()) {
            ...
            Icon icon = CordovaIcons.RenderError;
            ...
            msg = StringUtils.wrap(msg, charsPerLine, null);
            //noinspection UseJBColor
            ...
            painter.draw(gc, new PairFunction<Integer, Integer, Couple<Integer>>() {
                ...
                public Couple<Integer> fun(Integer width, Integer height) {
                    return (Couple)Pair.create(xf, yf);
                }
            });
            gc.setClip(prevClip);
        } else {
            ...
            gc.setColor(Color.DARK_GRAY);
            gc.drawRect(x, y, width, height);
            Icon icon = CordovaIcons.Toolbar.RefreshPreview;
            Composite prevComposite = gc.getComposite();
        ...
        if (myActive && !myShowFrame) {
            ...
            if (ZOOM_SUPPORT) {
                CordovaIcons.Toolbar.ZoomIn.paintIcon(component, gc, left, y);
                left += CordovaIcons.Toolbar.ZoomIn.getIconWidth();

                CordovaIcons.Toolbar.ZoomOut.paintIcon(component, gc, left, y);
                left += CordovaIcons.Toolbar.ZoomOut.getIconWidth();
            }
    ...
    int paintTitle(Graphics2D gc, int x, int y, boolean showFile, String displayName) {
        ...
        if (showFile && (myAlternateInput != null)) {
            // Draw file flag, and parent folder name
            VirtualFile file = myAlternateInput;
     ...
     *              {@code CHANGE_} constants in {@link RenderConfigurationListener}
     */
    public void configurationChanged(int flags) {
        ...
        if (DEBUG) {
            ...
            if (mode == RenderPreviewMode.DEFAULT) {
                assert myConfiguration instanceof VaryingRenderConfiguration;
                VaryingRenderConfiguration config = (VaryingRenderConfiguration) myConfiguration;
                int alternateFlags = config.getAlternateFlags();
                switch (alternateFlags) {
                    case RenderConfigurationListener.CFG_DEVICE_STATE: {
                    ...
                    case RenderConfigurationListener.CFG_DEVICE: {
                        Device configDevice = config.getDevice();
                        Device chooserDevice = configuration.getDevice();
                        assert configDevice != null && chooserDevice != null;
                        assert configDevice != chooserDevice : configDevice.toString() + ':' + chooserDevice;
