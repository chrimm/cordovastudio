...
public class TableCellBox extends BlockBox {
     ...
     */
    public TableCellBox(HtmlTag tag, Graphics2D g, VisualContext ctx) {
        super(tag, g, ctx);
        isblock = true;
        loadAttributes();
    ...
    public void applyVerticalAlign(int origHeight, int newHeight, int baseline) {
        ...
        switch (valign) {
            ...
            case MIDDLE:
                ...
                break;
            default:
                yofs = baseline - getFirstInlineBoxBaseline(); //all other values should behave as BASELINE
                break;
        }
    ...
    protected void loadSizes(boolean update) {
        ...
        String widthattr = ((HtmlTag) getElement()).getAttributeValue("width"); //try to load from attribute
        if (widthattr != null && !widthattr.equals("")) {
        ...
        String heightattr = ((HtmlTag) getElement()).getAttributeValue("height"); //try to load from attribute
        CSSProperty.Height hprop = null;
        TermLengthOrPercent height = null;
        if (heightattr != null && !heightattr.equals("")) {
