...
public class RenderedPanel extends JPanel implements Disposable {
      ...
      ApplicationManager.getApplication().invokeLater(new Runnable() {
        ...
        public void run() {
          ...
          if (file != null) {
            ...
            FileEditorManager.getInstance(project).setSelectedEditor(file, CordovaDesignerEditorProvider.CORDOVA_DESIGNER_ID);
          }
        }
    ...
    if (image != null) {
      ...
      if (selectedViews != null && !selectedViews.isEmpty() && !myErrorPanel.isVisible()) {
        ...
        if (configuration != null) {
          ...
          if (device != null && device.isRound()) {
            Dimension screen = device.getScreenSize(configuration.getDeviceState().getOrientation());
            Rectangle m = fromModelToScreen(0, 0, screen.width, screen.height);
            if (m != null) {
  ...
  private class MyImagePanelWrapper extends JBLayeredPane {
    ...
    private void positionErrorPanel() {
      ...
      int width = getWidth();
      int size;
      if (RenderErrorPanel.SIZE_ERROR_PANEL_DYNAMICALLY) {
