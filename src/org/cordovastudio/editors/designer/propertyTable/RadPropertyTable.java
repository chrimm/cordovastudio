...
import org.cordovastudio.editors.designer.componentTree.QuickFixManager;
...
public class RadPropertyTable extends PropertyTable implements DataProvider, ComponentSelectionListener {
  ...
  public void setArea(@Nullable CordovaDesignerEditorPanel designer, @Nullable IEditableArea area) {
    myDesigner = designer;
  ...
  protected boolean doRestoreDefault(ThrowableRunnable<Exception> runnable) {
    return myDesigner.getToolProvider().execute(runnable, CordovaDesignerBundle.message("designer.properties.restore_default"), false);
  }

  @Override
  protected boolean doSetValue(ThrowableRunnable<Exception> runnable) {
    return myDesigner.getToolProvider().execute(runnable, CordovaDesignerBundle.message("command.set.property.value"), false);
  }
      ...
      try {
        ...
        if (value == null) {
          transferable = new TextTransferable("");
            ...
            //TODO: fix!
          transferable = new TextTransferable("");//property.doCopy(myContainers.get(0), value);
        }
