...
public class ImageUtils {
     ...
     *                    crop to continue. This can be used to crop an image where you already
     *                    know about margins in the image
     * @param imageType   the type of {@link java.awt.image.BufferedImage} to create
     * @return a cropped version of the source image, or null if the whole image was blank
     * and cropping completely removed everything
     */
    @Nullable
     ...
     *                    crop to continue. This can be used to crop an image where you already
     *                    know about margins in the image
     * @param imageType   the type of {@link java.awt.image.BufferedImage} to create, or -1 if unknown
     * @return a cropped version of the source image, or null if the whole image was blank
     * and cropping completely removed everything
     */
    @Nullable
    ...
    /**
     * Loads an Icon from a Class name.
     * For example: {@code CordovaIcons.Designer.ToolWindow}
     * @param path The Class Path, either full qualified or beginning with {@code CordovaIcons.}
     * @return The Icon as specified, or null if not found.
     * @author Christoffer T. Timm <kontakt@christoffertimm.de>
     */
    @Nullable
    public static Icon getReflectiveIcon(@NotNull String path) {
        try {

            if(path.startsWith("CordovaIcons")) {
                if(path.contains(".")) {
                    path = "CordovaIcons$" + path.substring(path.indexOf(".") + 1);
                }
                /* Change path to FQCN */
                path = CordovaIcons.class.getPackage().getName() + "." + path;
            }

            Class cur = Class.forName(path.substring(0, path.lastIndexOf('.')));
