...
public class SelectorImpl extends AbstractRule<Selector.SelectorPart> implements Selector {
    ...
    public boolean matches(HtmlTag tag) {
        ...
        for (SelectorPart item : list) {
            if (item == null || !item.matches(tag, CSSFactory.getDefaultMatchCondition())) //null in case of syntax error (missing term)
                return false;
        }
    ...
    public boolean matches(HtmlTag tag, MatchCondition cond) {
        ...
        // check other items of simple selector
        for (SelectorPart item : list) {
            if (item == null || !item.matches(tag, cond)) //null in case of syntax error (missing term)
                return false;
        }
        // we passed checking
        return true;
    ...
    public static class ElementNameImpl implements ElementName {
        ...
        public boolean matches(HtmlTag tag, MatchCondition cond) {
            ...
            return TagUtil.matchesName(tag, name);
        }
    ...
    public static class ElementClassImpl implements ElementClass {
        ...
        public boolean matches(HtmlTag tag, MatchCondition cond) {
            return TagUtil.matchesClass(tag, className);
        }
    ...
    public static class PseudoPageImpl implements PseudoPage {
        ...
        public boolean matches(HtmlTag tag, MatchCondition cond) {
            ...
            if (declaration != null) { //null declaration means some unknown or unimplemented pseudo
                switch (declaration) {
                    ...
                    case NTH_LAST_CHILD:
                        return positionMatches(countSiblingsAfter(tag, false) + 1, elementIndex);
                    case NTH_OF_TYPE:
                        return positionMatches(countSiblingsBefore(tag, true) + 1, elementIndex);
                    case NTH_LAST_OF_TYPE:
                        return positionMatches(countSiblingsAfter(tag, true) + 1, elementIndex);
                    case ROOT:
                    ...
                    default:
                        ...
                        if (declaration.isPseudoElement() || cond.isSatisfied(tag, this)) {
                            return true;
                        }
                }
            }
         ...
         *
         * @param tag      The element to be examined
         ...
         */
        protected int countSiblingsBefore(HtmlTag tag, boolean sameType) {
            int cnt = 0;
            HtmlTag prev = tag;
            do {
                prev = (HtmlTag) prev.getPrevSibling();
                if (prev != null) {
                    if (!sameType || isSameElementType(tag, prev))
                        cnt++;
                }
            } while (prev != null);
         ...
         */
        protected int countSiblingsAfter(HtmlTag tag, boolean sameType) {
            int cnt = 0;
            HtmlTag next = tag;
            do {
                next = (HtmlTag) next.getNextSibling();
                if (next != null) {
                    if (!sameType || isSameElementType(tag, next))
                        cnt++;
                }
            } while (next != null);
    ...
    public static class ElementIDImpl implements ElementID {
        ...
        public boolean matches(HtmlTag tag, MatchCondition cond) {
            return TagUtil.matchesID(tag, id);
        }
    ...
    public static class ElementAttributeImpl implements ElementAttribute {
        ...
        public boolean matches(HtmlTag tag, MatchCondition cond) {
            return TagUtil.matchesAttribute(tag, attribute, value, operator);
        }
